...
const DepartmentProvider = ({ children }) => {
  ...
  async function getSelectDepartment(value) {
    const res = await sendRequest({
      ...
      url:
        value === "uz"
          ? "/api/departament/selecteddepartament"
          : `/api/departament/selectdepartamenttranslation?language_code=${value}`,
  ...
  async function getDepartmentType(value) {
    const res = await sendRequest({
      ...
      url:
        value === "uz"
          ? "/api/deartamenttypecontroller/sitegetalldepartamentType"
          : `/api/deartamenttypecontroller/sitegetalldepartamentTypetranslation?language_code=${value}`,
    });
  ...
  async function getSidebarDepartment() {
    const res = await sendRequest({
      method: "get",
      url: "/api/deartamenttypecontroller/sitegetalldepartamentType",
      headers: { Authorization: token },
    });
    if (res.status === 200) {
