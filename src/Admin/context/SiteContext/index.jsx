...
const SiteContextProvider = ({ children }) => {
  ...
  const getSiteData = async (value) => {
    const res = await axios.get(
      value === "uz"
        ? "/api/site/sitegetallsite"
        : `/api/site/sitegetallsitetranslation?language_code=${value}`,
      {
        headers: {
  ...
  const getSiteTypeData = async (value) => {
    const res = await axios.get(
      value === "uz"
        ? "/api/sitetype/sitegetallsitetype"
        : `/api/sitetype/sitegetallsitetypetranslation?language_code=${value}`,
      {
        headers: {
          Authorization: `Bearer ${Cookies.get("_token")}`,
        },
