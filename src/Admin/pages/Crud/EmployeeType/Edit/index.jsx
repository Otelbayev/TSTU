...
const Edit = () => {
  ...
  const onHandleSubmit = async (e) => {
    ...
    const res = await useEdit(
      ...
      "/api/employeetype/updateemployeetype",
      "/api/employeetype/updateemployeetypetranslation",
      [{ employee_id: id }, { language_id }, { status_translation_id: status }],
      ["status_id", "id"],
      "/api/employeetype/createemployeetypetranslation",
  ...
  const getData = async (value, id) => {
    const res = await axios.get(
      value === "uz"
        ? `/api/employeetype/getbyidemployeetype/${id}`
        : `/api/employeetype/getbyuzidemployeetypetranslation/${id}?language_code=${value}`,
      {
        headers: {
