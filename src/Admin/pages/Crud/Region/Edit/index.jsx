...
const Edit = () => {
    ...
    if (!isCreate) {
      try {
        const res = await sendRequest({
          ...
          url:
            value === "uz"
              ? `/api/territorie/updateterritorie/${id}`
              : `/api/territorie/updateterritorietranslation/${transId}`,
          data:
            value === "uz"
              ? {
                  title: type,
        ...
        if (res.status === 200) {
          ...
          navigate(`/${language}/admin/region`);
        }
      } catch (err) {
        message.error({
          key: "key",
      ...
      try {
        const res = await sendRequest({
          method: "post",
          url: "/api/territorie/createterritorietranslation",
          data: {
            title: type,
            language_id,
            territorie_id: id,
  ...
  useEffect(() => {
    ...
    value === "uz"
      ? getDataById(`/api/territorie/getbyidterritorie/${id}`)
      : getDataById(
          `/api/territorie/getbyuzidterritorietranslation/${id}?language_code=${value}`
        );
  }, [value]);
