...
const Edit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await useEdit(
      ...
      "/api/site/updatesite",
      "/api/site/updatesitetranslation",
      [
        { language_id },
      ...
      "/api/site/createsitetranslation",
      [
        { language_id },
        { site_id: id },
        { site_type_translation_id: siteTypeId },
    ...
    res?.status === 200 && navigate(`/${language}/admin/site`);
  };

  const getData = async () => {
    const res = await axios.get(
      value === "uz"
        ? `/api/site/getbyidsite/${id}`
        : `/api/site/getbyuzidsitetranslation/${id}?language_code=${value}`,
      {
        headers: {
