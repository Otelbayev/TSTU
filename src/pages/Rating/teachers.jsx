...
const Teachers = () => {
  ...
  useEffect(() => {
    setBreadcrumb([
      { name: t("faculties.all"), path: "/rating" },
      {
        name: t("faculties.kafedra"),
        path: `/rating/${window.location.pathname.split("/")[3]}`,
      },
      {
        name: t("about.professor"),
        path: `/rating/${window.location.pathname.split("/")[3]}/${id}`,
      },
    ]);
    window.scrollTo(0, 0);
  }, [data]);
